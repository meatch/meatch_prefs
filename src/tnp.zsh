...
# --------------------------------------------------------------
function tnpRebuild() {
    docker compose rm -f www webpack nginx
    docker compose build www nginx
    docker compose up -d --no-deps --force-recreate www
    docker compose up -d --no-deps --force-recreate webpack
...
function tnpBlackCheck() {
    # Usage:
    #   tnpBlackCheck                      # check entire repo (like CI)
    #   tnpBlackCheck noun_project/...py   # check a specific file or directory
    #   tnpBlackCheck . apply              # apply formatting fixes to entire repo
    #   tnpBlackCheck some/file.py apply   # apply formatting to specific file
    local target="${1:-.}"
    local mode="${2:-check}"  # 'check' or 'apply'

    # Determine black flags based on mode
    local black_flags
    if [ "$mode" = "apply" ]; then
        black_flags=""  # Just format, no --check
        echo ">>> Running Black (APPLY MODE) for: $target"
    else
        black_flags="--check --diff"
        echo ">>> Running Black (CHECK MODE) for: $target"
    fi
        ...
        if docker exec noun-www-1 sh -lc 'command -v black >/dev/null 2>&1'; then
            ...
            for base in "${bases[@]}"; do
                if docker exec noun-www-1 sh -lc "[ -e '${base}/${target}' ] || [ '${target}' = '.' ]"; then
                    echo "(container) black $black_flags ${base:+(cd $base && )}$target"
                    if [ -n "$base" ] && [ "$target" != "." ]; then
                        docker exec -it noun-www-1 sh -lc "cd '$base' && black --version && black $black_flags '$target'"
                    elif [ -n "$base" ] && [ "$target" = "." ]; then
                        docker exec -it noun-www-1 sh -lc "cd '$base' && black --version && black $black_flags ."
                    else
                        docker exec -it noun-www-1 sh -lc "black --version && black $black_flags '$target'"
                    fi
                    return $?
                fi
            done
